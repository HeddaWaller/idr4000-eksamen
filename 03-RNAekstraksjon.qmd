...
Resultat fra qPCR er vist i Tabell 1. Fortynningsrekke 1, 1/10 og 1/1000 hadde lite varians mellom Ct1, Ct2 og Ct3 med en variasjonskoeffisient (CV) på 1%. Fortynningsrekke 1/100 viste også lite variasjon mellom Ct1, Ct2 og Ct3 med CV på 2%. I fortynningsrekke 1/10000 ser vi derimot at Ct3 skilte seg betydelig fra Ct1 og Ct2 (34,927 vs 30,0 og 30,2). Dette resulterte i en relativt høy CV på 9%. Resultatene fra Tabell 1 ga en primereffektivitet på 86,54% og R^2 på 0,91.
...
numeric_cols <- c("Ct1", "Ct2", "Ct3", "Ct_gj_snitt", "Log_sample_quant", "SD", "CV_prosent")
df[numeric_cols] <- lapply(df[numeric_cols], function(x) as.numeric(gsub("[*]", "", x)))

gt_table <- df %>%
  gt() %>%
  tab_header(
    title = "Tabell 1: Resultat fra qPCR"
  ) %>%
  fmt_number(
...
gentabell <- genuttrykk %>%
  gt() %>%
  tab_spanner(
    label = "Tabell 2: Genuttrykk",
...
#| warning: false
#| message: false
#| fig-cap: "Figur 1: qPCR modellen viser sammenhengen mellom gjennosmnittlig Ct og log-transformasjonen av mengdemålingen (sample quant)."

library(ggplot2)

# Lag dataene
data <- data.frame(x = c(25.88092931, 27.4033076, 28.34053802, 29.13029989, 31.71848996, 30.91583951),
                   y = c(0.000, -0.301, -0.602, -0.903, -1.204, -1.505))

# Utfør lineær regresjon
linear_model <- lm(y ~ x, data = data)

# Hent stigningstallet, skjæringspunktet og r^2
...
#Lage likningen
r_squared_des <- sprintf("R^2 = %.4f", r_squared)
equation <- sprintf("y = %.3f + %.3fx", intercept, slope)

ggplot(data.frame(x = c(25.88092931, 27.4033076, 28.34053802, 29.13029989, 31.71848996, 30.91583951), 
                  y = c(0.000, -0.301, -0.602, -0.903, -1.204, -1.505)), 
       aes(x, y)) +
        geom_point(size = 2) +
        scale_y_continuous(limits = c(-1.60, 0)) +
        scale_x_continuous(limits = c(25, 33)) +
  
  # Legg til navn på x-aksen
  xlab("avg Ct") +  
  ylab("Log (sample quant)") + 
  geom_smooth(method = "lm",
              se = FALSE,
              color = "black") +
  
  #Legger inn slope og intercept
  geom_text(x = 29, 
            y = -1.4, 
            label = equation, 
            hjust = 0, 
